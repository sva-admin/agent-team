...
Haiku plus Sonnet share, the team is mis-routed. That's the whole diagnostic.
...
| Skipped files, didn't run tests, didn't verify | thoroughness gap | effort | raise effort, keep the model |
...
- Cost isn't monotonic in tier once effort is variable. A low-effort Fable call
...
is capped at Opus. Every agent file needs an explicit `model:` or you're
